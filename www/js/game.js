...
$("#start-game").click(function () {
    alert("dddd");
    unBindEvents();
    //reset the total time every time
    maxtime = 60 * 60;
    Clear();
    //run setInterval function for one time only
    if (staticTimer == 0) {
        staticTimer++;
        StartTimer();
        //bind mouse events with game to make pictures clickable, then player can start playing
        bindEvent();
    }
});
...
*/
//bind mouseover, mouseout, mouseup events with each picture. Must click Go button to bind these events before playing game.
var bindEvent = function () {
    ...
    //unbind mouseover, mouseout, mouseup events with each picture. So these pictures are not clickable.
...
//reset timer, score and all pictures
function Reset() {
...
//when click on a picture, change to another picture
...
//when mouse over a picture, change it to another picture
...
//when mouse out a picture, change it to another picture
